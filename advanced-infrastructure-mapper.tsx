...
export default function AdvancedInfrastructureMapper() {
  ...
  const [modalImpactEffects, setModalImpactEffects] = useState<ImpactEffect[]>(
    []
  );
  const [modalOtherImpactEffect, setModalOtherImpactEffect] = useState("");

  // Infrastructure element modal state
  const [pendingElement, setPendingElement] = useState<{
    id: string;
    name: string;
  } | null>(null);
  const [showElementModal, setShowElementModal] = useState(false);
  const [modalInfrastructureProblems, setModalInfrastructureProblems] =
    useState<InfrastructureProblem[]>([]);
  const [modalOtherInfrastructureProblem, setModalOtherInfrastructureProblem] =
    useState("");
